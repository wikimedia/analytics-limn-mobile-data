select sum(if(event_displaymobile = 1, 1, 0)) as mobile,
  ...
  from  (SELECT user_id
           FROM (SELECT rev_user as user_id,
                  ...
                  WHERE rev_timestamp BETWEEN DATE_FORMAT(DATE_SUB('{to_timestamp}', INTERVAL 30 DAY), "%Y%m%d%H%i%S") AND '{to_timestamp}'
                    AND rev_user in (select log_user
                                       from enwiki.logging
                                      where log_type = 'newusers'
                                        and log_action = 'create'
                                        and log_timestamp BETWEEN DATE_FORMAT(DATE_SUB('{to_timestamp}', INTERVAL 30 DAY), "%Y%m%d%H%i%S") AND '{to_timestamp}'
                                    )
                  GROUP BY rev_user
                 ...
                 SELECT ar_user as user_id,
                        count(*) AS revisions
                   FROM enwiki.archive
                  WHERE ar_timestamp BETWEEN DATE_FORMAT(DATE_SUB('{to_timestamp}', INTERVAL 30 DAY), "%Y%m%d%H%i%S") AND '{to_timestamp}'
        ...
        log.ServerSideAccountCreation_5487345  on event_userId = rolling_active_editors.user_id
                                               ...
                                               AND wiki = "enwiki"
                                               and timestamp between DATE_FORMAT(DATE_SUB('{to_timestamp}', INTERVAL 30+1 DAY), "%Y%m%d%H%i%S")
